...
onMounted(async () => {
  try {
    const res = await api.get(`api/cleaning_schedule/${cleaningScheduleId}/`);
    form.value = { ...res.data };
...
const submit = async () => {
  ...
  try {
    ...
    await api.put(`api/cleaning_schedule/${cleaningScheduleId}/`, form.value);
    router.push(`/cleaningSchedules/${cleaningScheduleId}`);
  } catch (err) {
